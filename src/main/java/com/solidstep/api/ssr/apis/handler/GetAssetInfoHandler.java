...
import com.solidstep.api.ssr.parameters.request.GetAssetInfoRequest;
import com.solidstep.api.ssr.parameters.response.GetAssetInfoResponse;
import com.solidstep.api.ssr.utils.SSRApiUtils;

public class GetAssetInfoHandler extends SSRApiHandler {
	
	@Override
	public SSRApiResult<GetAssetInfoResponse> handle(Object ssrRequest) {
		GetAssetInfoRequest getAssetInfoRequest = (GetAssetInfoRequest)ssrRequest;
		String apiResultString = this.ssrApiCall(getAssetInfoRequest);
		JSONObject resultInfoGenerate = SSRApiUtils.resultInfoGenerate(apiResultString);
		String apiJsonResultKey = getAssetInfoRequest.getApiJsonResultKey();
		List<Object> resultList = SSRApiUtils.listResultObjectMapping(GetAssetInfoResponse.class, apiResultString, apiJsonResultKey);
		SSRApiResult<GetAssetInfoResponse> result = new SSRApiResult<GetAssetInfoResponse>(resultList);
		try {
